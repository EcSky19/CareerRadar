...
import { scoreColor, formatRelative, STATUS_LABELS } from '@/lib/utils'
import { ExternalLink, Bookmark, BookmarkCheck, X, Filter } from 'lucide-react'
    ...
    <div className="flex h-full">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="section-header">
          <h1 className="page-title">Career Matches</h1>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-text-3 font-mono ml-1">{matches.length} results</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="p-12 text-center">
              <p className="text-text-3 text-sm">No matches found.</p>
            </div>
          ) : (
            <table className="data-table">
              <thead className="sticky top-0 bg-surface-1 z-10">
                <tr>
                  <th>Role</th><th>Score</th><th>Profile</th>
                  <th>Status</th><th>Seen</th><th></th>
                </tr>
              </thead>
              <tbody>
                {matches.map(m => (
                  <tr key={m.id} onClick={() => setSelectedMatch(m)}
                    ...
                    <td className="min-w-[260px]">
                      ...
                      <p className="text-text-3 text-2xs font-mono">
                        {m.company_name}{m.location && ` · ${m.location}`}{m.is_remote && ' · Remote'}
                      </p>
                    </td>
                    <td><ScoreBadge score={m.match_score} /></td>
                    <td><span className="pill-muted text-2xs">{m.profile_name}</span></td>
                    <td>
                      <select value={m.application_status}
                        onChange={e => updateAppStatus(m.job_id, e.target.value as ApplicationStatus)}
                        onClick={e => e.stopPropagation()}
                        className="text-2xs px-2 py-0.5 rounded border font-mono cursor-pointer bg-surface-3 border-surface-4 text-text-2">
                    ...
                    <td>
                      <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                        <button onClick={e => toggleSave(m, e)}
...
function MatchDetailPanel({ match, onClose, onStatusChange }: {
  match: JobMatch
  onClose: () => void
  onStatusChange: (jobId: string, status: ApplicationStatus) => void
}) {
  return (
    <aside className="w-[360px] shrink-0 border-l border-surface-4 bg-surface-2 flex flex-col overflow-y-auto">
      <div className="flex items-start justify-between p-5 border-b border-surface-4">
        ...
        <button onClick={onClose} className="p-1 rounded hover:bg-surface-4 shrink-0">
      ...
      <div className="p-5 space-y-5">
        <div className="flex items-center gap-4">
          ...
          <p className="text-xs text-text-2 leading-relaxed flex-1">{match.match_reason}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <DetailRow label="Remote"   value={match.is_remote ? 'Yes' : 'No'} />
          <DetailRow label="Profile"  value={match.profile_name} />
        </div>
        <div>
          <p className="text-2xs font-mono text-text-3 uppercase tracking-wider mb-2">Status</p>
          <select value={match.application_status}
...
function ScoreBadge({ score, large }: { score: number; large?: boolean }) {
  ...
  return (
    <span className={`score-badge font-mono ${size}`}
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1">
        {items.slice(0, 8).map(t => (
          <span key={t} className={`pill-${color}`}>{t}</span>
        ))}
      </div>
    </div>
  )
